...
#include "mgLinuxThread.h"

class LinuxThreadArgs
{
public:
  mgOSThread* m_thread;
...
void* LinuxThreadProc(
  void *argPtr)
{
  LinuxThreadArgs* args = (LinuxThreadArgs*) argPtr;
...
// =-= set priority
  ...
  {
    LinuxThreadArgs* args = new LinuxThreadArgs;
    args->m_thread = this;
    ...
    pthread_create(&m_threads[i], &attr, LinuxThreadProc, args);
  }
...
//--------------------------------------------------------------
// set priority
void mgLinuxThread::setPriority(
  int priority)
{
#ifdef WORKED
