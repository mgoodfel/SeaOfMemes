...
struct mgMapStringToDWordEntry
{
  char* key;
  DWORD value;
};
...
//--------------------------------------------------------------------
// constructor
mgMapStringToDWord::mgMapStringToDWord()
{
  m_tableSize = 97;
  m_tableCount = 0;
  m_entries = new mgMapStringToDWordEntry[m_tableSize];
...
//--------------------------------------------------------------------
// lookup a key
BOOL mgMapStringToDWord::lookup(
  const char* key, 
  DWORD& value) const
{
...
//--------------------------------------------------------------------
// remove a key
void mgMapStringToDWord::removeKey(
  const char* key)
{
  // hash the key for first probe into table
  int index = hash(key) % m_tableSize;
...
int mgMapStringToDWord::getStartPosition() const
{
...
void mgMapStringToDWord::getNextAssoc(
  int& posn, 
  mgString& key, 
  DWORD& value) const
{
...
//--------------------------------------------------------------------
// remove all keys
void mgMapStringToDWord::removeAll() 
{
  // delete all the entries
  for (int i = 0; i < m_tableSize; i++)
  {
    mgMapStringToDWordEntry *entry = (mgMapStringToDWordEntry *) &m_entries[i];
    if (entry->key != NULL)
    {
      delete entry->key;
      entry->key = NULL;
...
//--------------------------------------------------------------------
// grow the table
void mgMapStringToDWord::grow()
{
  int newSize = 1+2*m_tableSize;
  mgMapStringToDWordEntry* newEntries = new mgMapStringToDWordEntry[newSize];
  ...
  mgMapStringToDWordEntry* oldEntries = m_entries;
  ...
  {
    mgMapStringToDWordEntry *entry = &oldEntries[i];
    if (entry->key != NULL)
    {
