...
//--------------------------------------------------------------
// constructor
mgSimpleConsole::mgSimpleConsole(
  mgControl* parent,
  const char* cntlName)
: mgConsoleControl(parent, cntlName)
{
  ...
  style->getFontAttr(MG_STYLE_CONSOLE, getName(), "font", m_font);

  style->getFrameAttr(MG_STYLE_CONSOLE, getName(), "frame", m_frame);
  style->getColorAttr(MG_STYLE_CONSOLE, getName(), "textColor", m_textColor);
...
mgSimpleConsole::~mgSimpleConsole()
{
  ...
  {
    mgSimpleLine* msg = (mgSimpleLine*) m_lines->getAt(i);
    delete msg;
  }
  m_lines->removeAll();
...
//--------------------------------------------------------------
// format a line
void mgSimpleConsole::formatLine(
  int formatWidth,
  const mgColor& color,
  const mgFont* font,
  const char* text,
  int textLen)
{
  ...
  while (posn < textLen)
  {
    ...
    mgSimpleLine* line = new mgSimpleLine();
    line->m_color = color;
    line->m_font = font;
  ...
  const char* message)
{
  ...
  {
    mgSimpleLine* line = (mgSimpleLine*) m_lines->getAt(0);
    m_lines->removeAt(0);
    delete line;
  }
...
void mgSimpleConsole::reformatLines(
  int formatWidth)
{
//double start = mgOSGetTime();
  ...
  {
    mgSimpleLine* line = (mgSimpleLine*) oldLines->getAt(i);
...
//--------------------------------------------------------------
// set max height
void mgSimpleConsole::setDisplayLines(
  int lines)
{
...
//--------------------------------------------------------------
// set history
void mgSimpleConsole::setHistoryLines(
  int lines)
{
...
//--------------------------------------------------------------
// return minimum size
void mgSimpleConsole::minimumSize(
  mgDimension& size)
{
...
void mgSimpleConsole::preferredSize(
  mgDimension& size)
{
...
void mgSimpleConsole::updateLayout()
{
  mgDimension size;
  getSize(size);
...
//double start = mgOSGetTime();
  ...
  {
    mgSimpleLine* line = (mgSimpleLine*) m_lines->getAt(i);
...
//--------------------------------------------------------------
// update scrollbars
void mgSimpleConsole::updateScrollers()
{
  // figure new range
  if (m_vertScroller != NULL)
  {
...
//--------------------------------------------------------------
// line up
void mgSimpleConsole::guiScrollLineUp(
  void* source)
{
...
//--------------------------------------------------------------
// line down
void mgSimpleConsole::guiScrollLineDown(
  void* source)
{
  if (source == (mgScroller*) m_vertScroller)
  {
...
//--------------------------------------------------------------
// page up
void mgSimpleConsole::guiScrollPageUp(
  void* source)
{
  if (source == (mgScroller*) m_vertScroller)
  {
...
//--------------------------------------------------------------
// page down
void mgSimpleConsole::guiScrollPageDown(
  void* source)
{
  if (source == (mgScroller*) m_vertScroller)
  {
...
//--------------------------------------------------------------
// set position
void mgSimpleConsole::guiScrollSetPosition(
  void* source,
  int value)
{
  if (source == (mgScroller*) m_vertScroller)
