...
//--------------------------------------------------------------
// constructor
mgSimpleSplit::mgSimpleSplit(
  mgControl* parent,
  const char* cntlName)
: mgSplitControl(parent, cntlName)
{
//  m_vertical = vertical;
/*
  // get divider width
...
*/  
  ...
  style->getFontAttr(MG_STYLE_SPLIT, getName(), "font", m_font);

  // get divider width
  m_dividerWidth = m_font->stringWidth("n", 1);
  
  style->getFrameAttr(MG_STYLE_SPLIT, getName(), "divider", m_divider);
...
void mgSimpleSplit::setVertical(
  BOOL vertical)
{
  m_vertical = vertical;
...
mgControl* mgSimpleSplit::addPane(
  int weight)
{
...
void mgSimpleSplit::resizePanes()
{
  mgDimension size;
  getSize(size);
...
void mgSimpleSplit::minimumSize(
  mgDimension& size) 
{
  // sum of children minimum sizes
  size.m_width = 0;
  size.m_height = 0;
  for (int i = 0; i < m_panes.length(); i++)
  {
    mgPaneEntry* pane = (mgPaneEntry*) m_panes.getAt(i);
    mgDimension paneSize;
    pane->m_control->minimumSize(paneSize);
    if (m_vertical)
    {
...
void mgSimpleSplit::preferredSize(
  mgDimension& size) 
{
...
//--------------------------------------------------------------
// set control size
void mgSimpleSplit::updateLayout()
{
  resizePanes();
}
