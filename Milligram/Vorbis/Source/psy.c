...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    lo_curve= (int) ceilf(toOC(bin*binHz+1)*2);
    hi_curve= (int) floorf(toOC((bin+1)*binHz)*2);
...
void _vp_psy_init(vorbis_look_psy *p,vorbis_info_psy *vi,
  ...
  p->shiftoc= (long) (rint(log(gi->eighth_octave_lines*8.0f)/log(2.0f))-1);
  ...
  else if(rate < 38000) p->m_val = 0.94f;   /* 32kHz */
  else if(rate > 46000) p->m_val = 1.275f; /* 48kHz */

  /* set up the lookups for a given blocksize and sample rate */

  for(i=0,j=0;i<MAX_ATH-1;i++){
    int endpos=(int) rint(fromOC((i+1)*.125f-2.0f)*2*n/rate);
    float base=ATH[i];
    if(j<endpos){
  ...
  p->tonecurves=setup_tone_curves(vi->toneatt,rate*0.5f/n,n,
...
void _vp_couple_quantize_normalize(int blobno,
  ...
  float de=0.1f*p->m_val; /* a blend of the AoTuV M2 and M3 code here and below */
