  ...
  const char* fragmentFileName)
{
  ...
  if (!WebGLloadShaderFile(fileName, hVertexShader))
  {
  ...
  WebGLgetShaderLog(hVertexShader, log);
  log.trim();
  if (log.isEmpty())
  ...
  mgOSFixFileName(fileName);
  if (!WebGLloadShaderFile(fileName, hFragmentShader))
  {
    mgDebug("shader %s not found", (const char*) fileName);
    return false;
  }
  ...
  WebGLgetShaderLog(hFragmentShader, log);
  log.trim();
  if (log.isEmpty())
    mgDebug("shader %s compiled", (const char*) fileName);
  else mgDebug("shader %s log:\n%s", (const char*) fileName, (const char*) log);
...
{
  mgString NVshader;
  NVshader.format("%sWebGL/", (const char*) shaderDir);
  ...
  if (!WebGLcompileShaderPair(NVshader, hVertexShader, vertexFileName, hFragmentShader, fragmentFileName))
  ...
  if (testVal == GL_FALSE)
  {
    ...
    mgDebug("shader link: %s", (const char*) message);
    delete message;
...
// load and compile a shader
// =-= source code modified from a version in the OpenGL SuperBible
BOOL WebGLcompileShaderPairSource(
...
//--------------------------------------------------------------------------
// Load the shader file, with the supplied named attributes
GLuint mgWebGLloadShaderPairSource(
  const char* shaderName,
  const char* vertexSource, 
  const char* fragmentSource, 
  const mgVertexAttrib* attribs)
{
  ...
  // find a version which compiles on this display
  if (!WebGLcompileShaderPairSource(shaderName, hVertexShader, vertexSource, hFragmentShader, fragmentSource))
