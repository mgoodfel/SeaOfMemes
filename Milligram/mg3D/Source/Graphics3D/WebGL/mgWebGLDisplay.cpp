...
//--------------------------------------------------------------
// constructor
mgWebGLTextureImage::mgWebGLTextureImage()
{
...
inline GLuint mgWebGLTextureWrap(
  int mode)
{
  switch (mode)
  {
...
inline GLuint mgWebGLTextureFormat(
  int format)
{
  switch (format)
  {
    case MG_MEMORY_FORMAT_RGBA: return GL_RGBA;
    case MG_MEMORY_FORMAT_BGRA: return GL_RGBA;  // =-= not handled by WebGL
    case MG_MEMORY_FORMAT_GRAY: return GL_ALPHA;
...
void mgWebGLTextureImage::setWrap(
  int xWrap,
  int yWrap)
{
  m_xWrap = xWrap;
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, mgWebGLTextureWrap(m_xWrap));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, mgWebGLTextureWrap(m_yWrap));
}

//--------------------------------------------------------------
// set texture filter
void mgWebGLTextureImage::setFilter(
  int filter)
{
}
...
void mgWebGLTextureImage::updateMemory(
  int x,
  int y,
  int width,
  int height,
...
//  if (m_format != MG_MEMORY_FORMAT_RGBA)
//    throw new mgException("unsupported memory texture format.");

  glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, width, height, mgWebGLTextureFormat(m_format), GL_UNSIGNED_BYTE, data);
...
//--------------------------------------------------------------
// constructor
mgWebGLTextureArray::mgWebGLTextureArray()
{
  m_handle = mgNullHandle;
}
...
//--------------------------------------------------------------
// set texture wrap mode in x and y
void mgWebGLTextureArray::setWrap(
  int xWrap,
  int yWrap)
{
  m_xWrap = xWrap;
...
//  glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, mgWebGLTextureWrap(m_xWrap));
//  glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_T, mgWebGLTextureWrap(m_yWrap));
}

//--------------------------------------------------------------
// set texture filter
void mgWebGLTextureArray::setFilter(
  int filter)
{
}
...
//--------------------------------------------------------------
// constructor
mgWebGLTextureCube::mgWebGLTextureCube()
{
  m_handle = mgNullHandle;
}
...
//--------------------------------------------------------------
// set texture wrap mode in x and y
void mgWebGLTextureCube::setWrap(
  int xWrap,
  int yWrap)
{
  m_xWrap = xWrap;
  ...
  glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, mgWebGLTextureWrap(m_xWrap));
  glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, mgWebGLTextureWrap(m_yWrap));
}

//--------------------------------------------------------------
// set texture filter
void mgWebGLTextureCube::setFilter(
  int filter)
{
}
...
GLint mgWebGLShader::uniformIndex(
  const char* name)
{
  DWORD value;
...
mgWebGLDisplay::~mgWebGLDisplay()
{
  ...
  while (posn != -1)
  {
    ...
    mgWebGLTextureImage* texture = (mgWebGLTextureImage*) value;
    delete texture;
  }
  ...
  {
    mgWebGLTextureArray* texture = (mgWebGLTextureArray*) m_textureArrays[i];
    delete texture;
  }
  ...
  {
    mgWebGLTextureCube* texture = (mgWebGLTextureCube*) m_textureCubes[i];
    delete texture;
  }
  ...
  while (posn != -1)
  {
    ...
    mgWebGLShader* shader = (mgWebGLShader*) value;
    delete shader;
  }
  m_shaders.removeAll();
...
mgTextureSurface* mgWebGLDisplay::createOverlaySurface()
{
...
//--------------------------------------------------------------
// create a 2D graphics surface
mgTextureSurface* mgWebGLDisplay::createTextureSurface()
{
...
//--------------------------------------------------------------
// render to a texture
void mgWebGLDisplay::renderToTexture(
  mgTextureImage* target,
  BOOL withDepth)
{
  mgWebGLTextureImage* ourTexture = (mgWebGLTextureImage*) target;
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ourTexture->m_handle, 0);
...
void mgWebGLDisplay::renderToDisplay()
{
...
BOOL mgWebGLDisplay::supportsIntegerVertex()
{
  return false;
}
...
BOOL mgWebGLDisplay::canRepeatTextures()
{
  return false;
}
...
void mgWebGLDisplay::deleteBuffers()
{
  unloadTextures();
}
...
void mgWebGLDisplay::createBuffers()
{
  initView();
...
//--------------------------------------------------------------
// set projection
void mgWebGLDisplay::setProjection(
  int width,
  int height)
{
...
//-----------------------------------------------------------------------------
// load shader
mgShader* mgWebGLDisplay::loadShader(
  ...
  const mgVertexAttrib* attribs)      // vertex attributes
{
  ...
  mgShaderHandle handle = mgWebGLloadShaderPair(m_shaderDir, vertexShader, fragmentShader, attribs);

  mgWebGLShader* shader = new mgWebGLShader(shaderName);
...
//-----------------------------------------------------------------------------
// load shader
mgShader* mgWebGLDisplay::loadShaderSource(
  const char* shaderName,             // name of shader
  ...
  const mgVertexAttrib* attribs)      // vertex attributes
{
  ...
  mgShaderHandle handle = mgWebGLloadShaderPairSource(shaderName, vertexSource, fragmentSource, attribs);
...
//-----------------------------------------------------------------------------
// delete shader
void mgWebGLDisplay::deleteShader(
  mgShader* shader)                           // shader instance
{
  mgWebGLShader* ourShader = (mgWebGLShader*) shader;
...
//-----------------------------------------------------------------------------
// set current shader
void mgWebGLDisplay::setShader(
  mgShader* shader)                           // shader instance
{
  m_shader = (mgWebGLShader*) shader;
...
void mgWebGLDisplay::setShaderUniform(
  mgShader* shader,                   // shader instance
...
void mgWebGLDisplay::setShaderStdUniforms(
...
void mgWebGLDisplay::matrix4toGL(
  const mgMatrix4& m,
  float* values)
{
...
void mgWebGLDisplay::matrix3toGL(
  const mgMatrix3& m,
  float* values)
{
  values[0] = (float) m._11;
...
void mgWebGLDisplay::normalMatrix(
...
void mgWebGLDisplay::drawOverlayTexture(
  ...
  int height)
{
  mgWebGLTextureImage* ourTexture = (mgWebGLTextureImage*) texture;
  if (mgPlatform != NULL)
...
void mgWebGLDisplay::drawOverlaySurface(
...
void mgWebGLDisplay::drawCursor()
{
...
//--------------------------------------------------------------
// draw the view
void mgWebGLDisplay::clearBuffer(
  int flags)
{
  GLbitfield mask = 0;
...
mgVertexBuffer* mgWebGLDisplay::newVertexBuffer(
...
{
  return new mgWebGLVertexBuffer(vertexSize, attribs, maxVertexes, dynamic);
}

//--------------------------------------------------------------
// allocate an index array
mgIndexBuffer* mgWebGLDisplay::newIndexBuffer(
  int size,                         // max number of indexes
  BOOL dynamic,                     // support reset and reuse
  BOOL longIndexes)                 // support 32-bit indexes
{
  return new mgWebGLIndexBuffer(size, dynamic, longIndexes);
}

//--------------------------------------------------------------
// set the texture to use
void mgWebGLDisplay::setTexture(
  const mgTextureImage* texture,
  int unit)
{
  mgWebGLTextureImage* ourTexture = (mgWebGLTextureImage*) texture;
...
void mgWebGLDisplay::setTexture(
  ...
  int unit)
{
  mgWebGLTextureArray* ourTextureArray = (mgWebGLTextureArray*) textureArray;
...
void mgWebGLDisplay::setTexture(
  ...
  int unit)
{
  mgWebGLTextureCube* ourTextureCube = (mgWebGLTextureCube*) textureCube;
...
void mgWebGLDisplay::useVertexBuffer(
  ...
  mgVertexBuffer* triangles)
{
  ...
  mgWebGLVertexBuffer* ourVertexes = (mgWebGLVertexBuffer*) triangles;
  ...
  glDrawArrays(mgWebGLprimType(primType), 0, ourVertexes->m_count);
  ...
  int endIndex)
{
  ...
  glDrawArrays(mgWebGLprimType(primType), startIndex, endIndex-startIndex);
  ...
  mgIndexBuffer* indexes)
{
  ...
  mgWebGLVertexBuffer* ourVertexes = (mgWebGLVertexBuffer*) vertexes;
  mgWebGLIndexBuffer* ourIndexes = (mgWebGLIndexBuffer*) indexes;
  ...
  glDrawElements(mgWebGLprimType(primType), ourIndexes->m_count, indexSize, (const GLvoid*) 0);
  ...
  int endIndex)
{
  ...
  glDrawElements(mgWebGLprimType(primType), endIndex-startIndex, indexType, (const GLvoid*) startOffset);
...
void mgWebGLDisplay::decalBackground(
  BOOL draw)
{
...
void mgWebGLDisplay::decalStart()
{
  glDepthFunc(GL_LESS);
...
//--------------------------------------------------------------
// set world transform
void mgWebGLDisplay::setModelTransform(
...
void mgWebGLDisplay::appendModelTransform(
  const mgMatrix4& xform)
{
...
//--------------------------------------------------------------
// get world transform
void mgWebGLDisplay::getModelTransform(
  mgMatrix4& xform)
{
  xform = m_modelMatrix;
}
...
//--------------------------------------------------------------
// get mvp transform
void mgWebGLDisplay::getMVPTransform(
  mgMatrix4& xform)
{
  xform = m_worldMatrix;
...
//--------------------------------------------------------------
// get mv transform
void mgWebGLDisplay::getMVTransform(
  mgMatrix4& xform)
{
  xform = m_worldMatrix;
}
...
//--------------------------------------------------------------
// set light position
void mgWebGLDisplay::setLightDir(
  double x,
  double y,
  double z)
{
...
//--------------------------------------------------------------
// set light color
void mgWebGLDisplay::setLightColor(
  double r,
  double g,
  double b)
{
...
//--------------------------------------------------------------
// set light ambient
void mgWebGLDisplay::setLightAmbient(
  double r,
  double g,
  double b)
{
...
//-----------------------------------------------------------------------------
// set material color
void mgWebGLDisplay::setMatColor(
  double r,
  double g,
  double b,
  double a)
...
//--------------------------------------------------------------
// set zenable
void mgWebGLDisplay::setZEnable(
  BOOL enable)
{
  if (enable)
    glEnable(GL_DEPTH_TEST);
...
//--------------------------------------------------------------
// set culling 
void mgWebGLDisplay::setCulling(
  BOOL enable)
{
  if (enable)
    glEnable(GL_CULL_FACE);
...
void mgWebGLDisplay::setFrontCW(
  BOOL enable)
{
  if (enable)
    glFrontFace(GL_CW);
...
//--------------------------------------------------------------
// set transparent
void mgWebGLDisplay::setTransparent(
  BOOL enable)
{
  if (enable)
    glEnable(GL_BLEND);
...
//-----------------------------------------------------------------------------
// set cursor pattern
void mgWebGLDisplay::setCursorTexture(
  const char* fileName,
  int hotX,
  int hotY)
{
  ...
  m_cursorTexture = (mgWebGLTextureImage*) loadTexture(fileName);
}

//--------------------------------------------------------------
// scale texture array down by factor of 2.
void mgWebGLDisplay::scaleTextureImage(
  int width,
  int height,
  BYTE* data)
{
...
mgTextureImage* mgWebGLDisplay::loadTexture(
  const char* fileName)
{
  const void *value;
  ...
  mgWebGLTextureImage* texture = new mgWebGLTextureImage();
...
mgTextureArray* mgWebGLDisplay::loadTextureArray(
  const mgStringArray& fileList)   
{
  mgWebGLTextureArray* texture = new mgWebGLTextureArray();
...
mgTextureCube* mgWebGLDisplay::loadTextureCube(
...
{
  mgWebGLTextureCube* texture = new mgWebGLTextureCube();
...
mgTextureImage* mgWebGLDisplay::createTextureMemory(
  int width,
  int height,
  int format,
  BOOL mipmap)
...
//  if (format != MG_MEMORY_FORMAT_RGBA)
//    throw new mgException("unsupported memory texture format.");

  GLint texFormat = mgWebGLTextureFormat(format);
...
void mgWebGLDisplay::reloadTextureImage(
  mgWebGLTextureImage* texture)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, mgWebGLTextureWrap(texture->m_xWrap));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, mgWebGLTextureWrap(texture->m_yWrap));

  SDL_Surface* surface = IMG_Load(texture->m_fileName);

  texture->m_width = surface->w;
  texture->m_height = surface->h;
  texture->m_transparent = surface->format->BytesPerPixel == 4;
  GLint format = texture->m_transparent ? GL_RGBA : GL_RGB;
...
void mgWebGLDisplay::reloadTextureArray(
  mgWebGLTextureArray* texture)
{
  ...
  int cellAlpha = hasAlpha;
  int sourceDepth = surface->format->BytesPerPixel;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, atlasWidth, atlasHeight, 
  ...
  for (int i = 0; i < texture->m_fileList.length(); i++)
  {
    ...
    // copy the image into the altas
    glTexSubImage2D(GL_TEXTURE_2D, 0, col, row, cellWidth, cellHeight, GL_RGBA, 
...
void mgWebGLDisplay::reloadTextureCube(
  mgWebGLTextureCube* texture)
{
  ...
  glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, mgWebGLTextureWrap(texture->m_xWrap));
  glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, mgWebGLTextureWrap(texture->m_yWrap));
  ...
  surface = IMG_Load(texture->m_xminImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA, 
    arrayWidth, arrayHeight, 0, hasAlpha ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, surface->pixels);

  SDL_FreeSurface(surface);

  // load XMAX image 
  surface = IMG_Load(texture->m_xmaxImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
  ...
  // load YMIN image 
  surface = IMG_Load(texture->m_yminImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
  ...
  // load YMAX image 
  surface = IMG_Load(texture->m_ymaxImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
  ...
  // load ZMIN image 
  surface = IMG_Load(texture->m_zminImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
  ...
  // load ZMAX image 
  surface = IMG_Load(texture->m_zmaxImage);
  imgWidth = surface->w;
  imgHeight = surface->h;
  hasAlpha = surface->format->BytesPerPixel == 4;
...
void mgWebGLDisplay::unloadTextures()
{
...
void mgWebGLDisplay::reloadTextures()
{
...
void mgWebGLDisplay::loadOverlayShaders()
{
  mgString vs;
  ...
  fs += "  if (pixel.a == 0.0)\n";
  fs += "    discard;\n";
  fs += "  gl_FragColor = vec4(textColor.r, textColor.g, textColor.b, textColor.a * pixel.a);\n";
  fs += "}\n";
  ...
  fs += "  if (pixel.a == 0.0)\n";
  fs += "    discard;\n";
  fs += "  gl_FragColor = vec4(textColor.r, textColor.g, textColor.b, pixel.a);\n";
  fs += "}\n";
  ...
  fs += "  if (pixel.a == 0.0)\n";
  fs += "    discard;\n";
  // second pass.  dst a already set to dst.a*(1-pixel.a))
  // now add in color.a * pixel.a
  fs += "  gl_FragColor = vec4(0.0, 0.0, 0.0, textColor.a * pixel.a);\n";
  fs += "}\n";
...
void mgWebGLDisplay::getOverlayShaders(
