  ...
  const char* fragmentFileName)
{
  ...
  if (!GL33loadShaderFile(fileName, hVertexShader))
  {
  ...
  mgOSFixFileName(fileName);
  if (!GL33loadShaderFile(fileName, hFragmentShader))
  {
    mgDebug("shader %s not found", (const char*) fileName);
    return false;
  }
  ...
  ...)
{
  ...
  mgString ATIshader;
  ATIshader.format("%sGL33/ATI/", (const char*) shaderDir);
  ...
  if (!GL33compileShaderPair(NVshader, hVertexShader, vertexFileName, hFragmentShader, fragmentFileName))
    if (!GL33compileShaderPair(ATIshader, hVertexShader, vertexFileName, hFragmentShader, fragmentFileName))
