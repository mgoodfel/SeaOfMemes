...
#if defined(SUPPORT_WEBGL) && defined(EMSCRIPTEN)
...
//--------------------------------------------------------------
// initialize graphics
BOOL mgWebGLSupport::initDisplay()
{
  mgDebug("------ try to create WebGL context, fullscreen=%s, multiSample=%s", 
...
/*
  ...
  mgString errorMsg;
  if (!checkVersion(100, 100, errorMsg))
  {
...
//--------------------------------------------------------------
// terminate graphics
void mgWebGLSupport::termDisplay()
{
  mgScriptPlatform* platform = (mgScriptPlatform*) mgPlatform;

/*
  glXMakeCurrent(system->m_display, 0, 0);
  glXDestroyContext(system->m_display, system->m_glrc);
  system->m_glrc = NULL;
...
void mgWebGLSupport::swapBuffers()
{
  mgWebGLSupport* system = (mgWebGLSupport*) mgPlatform;

/*
  glXSwapBuffers(system->m_display, system->m_window);
*/
}

//--------------------------------------------------------------
// check for GL errors
BOOL mgWebGLSupport::checkError()
{
...
BOOL mgWebGLSupport::checkVersion(
  int reqGLVersion,
...
BOOL mgWebGLSupport::builtinShaders()
{
  mgString vs;
...
void mgWebGLSupport::getShaderLog(
  GLuint shader,
  mgString& log)
{
  GLint logLen, retLen;
...
GLuint mgWebGLSupport::compileShader(
  const char* source,
  GLenum shaderType)
{
...
DWORD mgWebGLSupport::compileShaderPair(
...
void mgWebGLSupport::drawOverlayTexture(
  DWORD texture,
  int x, 
  int y,
  int width,
