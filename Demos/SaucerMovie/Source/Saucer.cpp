  ...
  double since)
{
  ...
  // if fading away
  if (m_fade > 0 && m_fade < 100)
  {
    m_fade += since/10;

    // if completely gone, mark destroyed
    if (m_fade > 100)
      m_destroyed = true;
  }
  ...
  if (m_slow > 0 && m_slow < 100)
  {
    m_slow += since/90;
    if (m_slow > 100)
      m_slow = 100;
  }
  if (m_slow >= -100 && m_slow < 0)
  {
    m_slow += since/50;
    if (m_slow > 0)
      m_slow = 0;
  }
  double slow = 1.0 - (fabs(m_slow)/100);
  m_zAngle *= slow;
  m_xAngle *= slow;

  m_origin.z += slow * since/40;
...
void Saucer::render()
{
  ...
  double burn = 1.0 - (m_burnout / 100);
  double alpha = 1.0 - (m_fade / 100);
  mgDisplay->setMatColor(1.0, burn, burn, alpha);
