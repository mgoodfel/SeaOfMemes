...
double Terrain::heightAtPt(
  double tx, 
  double tz)
{
  double ht = noise(tx/40000.0, tz/40000.0);
  ht = 10.0*exp(5.0*fabs(ht));
  ht += 20*noise(tx/1333.0, tz/1333.0);
  return ht;
}
...
inline double Terrain::landscapeHeight(
  double tx, 
  double tz)
{
  double ht = noise(tx/40000.0, tz/40000.0);
  ...
  const mgPoint3& eyePt)
{
  ...
  if (m_children[0] == NULL)
  {
    if (tooCoarse(eyePt))
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        m_children[i]->checkResolution(eyePt);
      }
    }
  }
  else
  {
    ...
    {
      m_children[i]->checkResolution(eyePt);
      if (m_children[i]->m_children[0] != NULL || !m_children[i]->tooFine(eyePt))
        tooFine = false;
    }
...
//  mgDebug("split chunk (%d,%d) %d", m_originX, m_originZ, m_extentX);
  ...
  Terrain* chunk = new Terrain(m_originX, m_originZ, sizeX, sizeZ);
  chunk->extractHeightmapFrom(m_heights, 0, 0);
  m_children[0] = chunk;

  chunk = new Terrain(m_originX + sizeX, m_originZ, sizeX, sizeZ);
  chunk->extractHeightmapFrom(m_heights, TERRAIN_SIZE/2, 0);
  m_children[1] = chunk;

  chunk = new Terrain(m_originX, m_originZ + sizeZ, sizeX, sizeZ);
  chunk->extractHeightmapFrom(m_heights, 0, TERRAIN_SIZE/2);
  m_children[2] = chunk;

  chunk = new Terrain(m_originX + sizeX, m_originZ + sizeZ, sizeX, sizeZ);
...
void Terrain::extractHeightmapFrom(
...
//  mgDebug("combine chunk (%d,%d) %d", m_originX, m_originZ, m_extentX);
  ...
  for (int z = 0; z < 2; z++)
  {
    for (int x = 0; x < 2; x++)
    {
      ...
      int child = x + z*2;
      float* childHeights = m_children[child]->m_heights;
...
//  mgDebug("createTerrain in (%d, %d) %d", m_originX, m_originZ, m_extentX);
  ...
  for (int z = 0; z < vertexSize; z++)
  {
    for (int x = 0; x < vertexSize; x++)
    {
      ...
      switch (gridStyle)
      {
        ...
        case GRID_CHUNKS:
          v.m_tx = (float) x/TERRAIN_SIZE;
          v.m_ty = (float) z/TERRAIN_SIZE;
          break;
      }
      ...
      // instead, raise surface to zero and color like water.
      if (mapView || gridStyle != GRID_TERRAIN)
      {
        if (ht < WATER_LEVEL)
        {
          v.m_py = WATER_LEVEL;
  ...
  int x, z;
  // do all the interior triangles
  for (z = 1; z < TERRAIN_SIZE-1; z++)
  {
    for (x = 1; x < TERRAIN_SIZE-1; x++)
    {
  ...
  for (x = 1; x < TERRAIN_SIZE-1; x++)
  {
    // snap to low res points on top edge
    int edgeIndex = z*vertexSize+m_zminStep*(x/m_zminStep);
    int index = z*vertexSize+x;
    m_indexes->addIndex(edgeIndex);  // tl
    m_indexes->addIndex(edgeIndex+m_zminStep);  // tr
    m_indexes->addIndex(index+vertexSize);  // bl
    m_indexes->addIndex(index+vertexSize);  // bl
    m_indexes->addIndex(edgeIndex+m_zminStep);  // tr
    m_indexes->addIndex(index+vertexSize+1);  // br
  ...
  for (x = 1; x < TERRAIN_SIZE-1; x++)
  {
    // snap to low res points on bottom edge
    int edgeIndex = (z+1)*vertexSize+m_zmaxStep*(x/m_zmaxStep);
    int index = z*vertexSize+x;
    m_indexes->addIndex(index);  // tl
    m_indexes->addIndex(index+1);  // tr
    m_indexes->addIndex(edgeIndex);  // bl
    m_indexes->addIndex(edgeIndex);  // bl
    m_indexes->addIndex(index+1);  // tr
    m_indexes->addIndex(edgeIndex+m_zmaxStep);  // br
  }
  ...
  for (z = 1; z < TERRAIN_SIZE-1; z++)
  {
    // snap to low res points on left edge
    int edgeIndex = (m_xminStep*(z/m_xminStep))*vertexSize+x;
    int index = z*vertexSize+x;
    m_indexes->addIndex(edgeIndex);  // tl
    m_indexes->addIndex(index+1);  // tr
    m_indexes->addIndex(edgeIndex+vertexSize*m_xminStep);  // bl
    m_indexes->addIndex(edgeIndex+vertexSize*m_xminStep);  // bl
    m_indexes->addIndex(index+1);  // tr
    m_indexes->addIndex(index+vertexSize+1);  // br
  }
  ...
  {
    // snap to low res points on left edge
    int edgeIndex = (m_xmaxStep*(z/m_xmaxStep))*vertexSize+x+1;
    int index = z*vertexSize+x;
    m_indexes->addIndex(index);  // tl
    m_indexes->addIndex(edgeIndex);  // tr
    m_indexes->addIndex(index+vertexSize);  // bl
    m_indexes->addIndex(index+vertexSize);  // bl
    m_indexes->addIndex(edgeIndex);  // tr
    m_indexes->addIndex(edgeIndex+vertexSize*m_xmaxStep);  // br
  }
  ...
  int gridStyle)
{
  ...
  {
    if (m_vertexes == NULL)
      createTerrain(mapView, gridStyle);
  ...
  else
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      m_children[i]->createBuffers(mapView, gridStyle);
  ...
  mgTextureImage* highlight)
{
  ...
  if (m_children[0] == NULL)
  {
    ...
    if (now - m_age < 1500)
      mgDisplay->setTexture(highlight);
    else mgDisplay->setTexture(terrain);
  ...
  else
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      m_children[i]->render(renderEyePt, terrain, highlight);
    }                          
  }
}
...
void Terrain::renderTransparent(
  const mgPoint3& renderEyePt,
