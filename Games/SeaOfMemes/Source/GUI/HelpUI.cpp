  ...
  const mgOptionsFile& options)
{
  ...
  const mgPaint* helpBack = mgSolidPaint::createPaint(surface, mgColor(200/255.0, 200/255.0, 255/255.0));
  const mgFrame* helpFrame = mgRectFrame::createFrame(surface, mgFlatEdge, 2, mgColor("black"), helpBack, mgFrameMargin(10));
  ...
  int height)
{
  ...
  m_overlay->getSurface()->setSurfaceSize(size.m_width, size.m_height);
}
...
{
  return m_top != NULL && m_top->getKeyFocus() != NULL;
}
...
{
  return m_top != NULL && m_top->getMouseFocus() != NULL;
}
